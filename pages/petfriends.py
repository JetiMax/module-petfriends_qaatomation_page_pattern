...
class MainPage(WebPage):
    ...
    def __init__(self, web_driver, url=''):  # Use __init__ instead of init
        if not url:
            url = os.getenv("MAIN_URL") or 'https://petfriends.skillfactory.ru/'

        super().__init__(web_driver, url)

        with open('my_cookies.txt', 'rb') as cookies_file:
            cookies_data = pickle.load(cookies_file)

        for cookie in cookies_data:
            web_driver.add_cookie(cookie)
            web_driver.refresh()
#         """2ой вариант открыитя куков"""
#         with open(r'C:\Users\Maxim\PycharmProjects\petfriends_qaatomation_page_pattern\tests\my_cookies.txt', 'rb') as cookiesfile:
...
# class MainPage(WebPage):
#
#     def __init__(self, web_driver, url=''):  # Use init instead of init
#         if not url:
#             url = os.getenv("MAIN_URL") or 'https://petfriends.skillfactory.ru/'
#
#         super().__init__(web_driver, url)
#
#         with open(r'C:\Users\Maxim\PycharmProjects\petfriends_qaatomation_page_pattern\tests\my_cookies.txt',
#                   'rb') as cookies_file:
#             try:
#                 while True:
#                     cookie = pickle.load(cookies_file)
#                     if 'cookie' in cookie:
#                         web_driver.add_cookie(cookie)
#                     web_driver.refresh()
#             except EOFError:
#                 pass
    ...
    def go_to_all_pets_page(self):
        self._web_driver.get('https://petfriends.skillfactory.ru/all_pets')  # Navigate to the all pets page after authorization
    ...
    # Названия продуктов в результатах поиска
